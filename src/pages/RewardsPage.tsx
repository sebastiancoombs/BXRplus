...
function AddRewardForm({ clientId, onAdded }: { clientId: string; onAdded: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    await supabase.from("rewards").insert({
      client_id: clientId, name, point_cost: cost, icon,
      description: desc || null, is_active: true, created_by: null,
    });
    setName(""); setCost(10); setDesc(""); setBusy(false);
    onAdded();
  }
      ...
      <CardContent className="py-4">
        <form onSubmit={submit} className="flex flex-wrap gap-3 items-end">
          ...
          <div className="flex-1 min-w-[140px] space-y-1">
            ...
            <Input value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="15 min iPad" />
          </div>
          <Button type="submit" disabled={busy}>{busy ? "Adding..." : "Add Reward"}</Button>
        </form>
      </CardContent>
    </Card>
  );
...
function AddBehaviorForm({ clientId, onAdded }: { clientId: string; onAdded: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    await supabase.from("behaviors").insert({
      client_id: clientId, name, point_value: points, icon,
      description: desc || null, is_active: true, created_by: null,
    });
    setName(""); setPoints(1); setDesc(""); setBusy(false);
    onAdded();
  }
    ...
    <Card>
      <CardContent className="py-4">
        <form onSubmit={submit} className="flex flex-wrap gap-3 items-end">
          ...
          <div className="flex-1 min-w-[140px] space-y-1">
            <Label className="text-xs">Description</Label>
            <Input value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="Complied with prompt" />
          </div>
          <Button type="submit" disabled={busy}>{busy ? "Adding..." : "Add Behavior"}</Button>
        </form>
      </CardContent>
    </Card>
  );
